...
export default function GenericPageView() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7]" style={{ fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {blocks.length > 0
        ? groups.map((g, gi) => {
            if (g.kind === 'single') return <BlockRenderer key={g.block.id} block={g.block} t={DEFAULT_T} />;
            ...
            return (
              <div key={`pg-${gi}`} style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  {g.blocks.map((block: PageBlock) => <BlockRenderer key={block.id} block={block} t={DEFAULT_T} />)}
                </div>
              </div>
            );
          })
