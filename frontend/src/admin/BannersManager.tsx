...
import {
  ...
  ExternalLink, X, Layout, Image as ImageIcon, Info, Smartphone,
} from 'lucide-react';
...
import { AdminPage, AdminTopbar, AdminSaveStatus, AdminCard, AdminEmptyState, AdminField } from '@/components/admin/primitives';
...
export function BannersManager() {
  ...
  const handleUpload = async (file: File, field: 'image' | 'image_mobile' = 'image') => {
    if (!draft?.id) return;
    const setBusy = field === 'image_mobile' ? setUploadingMobile : setUploading;
    setBusy(true);
    try {
      ...
      const patch: Partial<Banner> = field === 'image_mobile'
        ? { image_mobile: url }
        : { image: url, use_default_bg: 0 };
      await saveNow(patch);
      toast({ title: field === 'image_mobile' ? 'Imagem mobile enviada!' : 'Imagem enviada!' });
    } catch (e: any) {
  ...
  return (
    <AdminPage>
      <AdminTopbar
        ...
        subtitle="Salvo automaticamente ao editar"
        status={<AdminSaveStatus status={saving ? 'saving' : 'idle'} />}
        actions={
          ...
          <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {pageBanners.length === 0 ? (
              <AdminEmptyState
                ...
                title="Nenhum banner"
                description="Crie o primeiro banner desta página."
                action={
                  <Button onClick={handleAdd} size="sm" style={{ background: '#f97316', color: '#fff', borderRadius: 10, gap: 4 }}>
        ...
        {!draft ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <AdminEmptyState
              ...
              description="Clique em um banner à esquerda para editar, ou crie um novo."
            />
          </div>
        ) : (
          <div style={{ flex: 1, overflowY: 'auto' }}>
            <div style={{ maxWidth: 760, margin: '0 auto', padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {/* ── Agendamento ── */}
              <AdminCard title="Agendamento (opcional)" description="Defina datas de início e fim para exibição automática do banner.">
              ...
              <div style={{ background: '#fff', borderRadius: 16, border: '1px solid rgba(0,0,0,.07)', padding: 20 }}>
                ...
                {draft.use_default_bg === 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div style={{ marginTop: 8, paddingTop: 16, borderTop: '1px dashed rgba(0,0,0,.08)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                        <div>
                          <p style={{ fontSize: 13, fontWeight: 700, color: '#1d1d1f', margin: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
                            <Smartphone size={14} /> Imagem Mobile <span style={{ fontWeight: 500, color: '#98989d', fontSize: 11 }}>(opcional)</span>
                          </p>
                          <p style={{ fontSize: 11, color: '#98989d', margin: '2px 0 0' }}>
                            Foto vertical otimizada para celular (≤ 768px). Se vazio, usa a desktop.
                          </p>
                        </div>
                      </div>

                      <div
                        style={{ position: 'relative', aspectRatio: '4/5', maxWidth: 200, borderRadius: 12, overflow: 'hidden', cursor: draft.image_mobile ? 'default' : 'pointer', background: '#111' }}
                        onClick={() => !draft.image_mobile && fileMobileRef.current?.click()}
                      >
                        {draft.image_mobile ? (
                          <>
                            <img src={imgSrc(draft.image_mobile)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            <div style={{
                              position: 'absolute', inset: 0, background: 'rgba(0,0,0,.55)',
                              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                              opacity: 0, transition: 'opacity .2s', flexDirection: 'column',
                            }}
                              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.opacity = '1'; }}
                              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.opacity = '0'; }}
                            >
                              <button onClick={e => { e.stopPropagation(); fileMobileRef.current?.click(); }}
                                style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 10px', borderRadius: 8, background: '#fff', fontSize: 11, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
                                <Upload size={11} /> {uploadingMobile ? 'Enviando...' : 'Trocar'}
                              </button>
                              <button onClick={e => { e.stopPropagation(); saveNow({ image_mobile: '' }); }}
                                style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '6px 10px', borderRadius: 8, background: '#ef4444', color: '#fff', fontSize: 11, fontWeight: 700, border: 'none', cursor: 'pointer' }}>
                                <X size={11} /> Remover
                              </button>
                            </div>
                          </>
                        ) : (
                          <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, border: '2px dashed rgba(255,255,255,.15)', borderRadius: 12, padding: 12, textAlign: 'center' }}>
                            {uploadingMobile
                              ? <RefreshCw size={20} color="#f97316" style={{ animation: 'spin 1s linear infinite' }} />
                              : <>
                                  <Smartphone size={20} color="#6b7280" />
                                  <p style={{ fontSize: 11, fontWeight: 600, color: '#9ca3af', margin: 0 }}>Enviar foto mobile</p>
                                </>
                            }
                          </div>
                        )}
                      </div>
                      <input ref={fileMobileRef} type="file" accept="image/*" style={{ display: 'none' }}
                        onChange={e => { const f = e.target.files?.[0]; if (f) handleUpload(f, 'image_mobile'); e.target.value = ''; }} />
                    </div>
              ...
              {/* ── Estilo visual ── */}
              <AdminCard title="Estilo Visual" description="Como o banner é renderizado no site.">
              ...
              <AdminCard
                title="Textos"
                description="Deixe qualquer campo vazio para não exibir."
                icon={<Info size={14} />}
              >
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
                  <AdminField label="Título" required counter={{ current: (draft.title || '').length, max: 200 }}>
                    <Input value={draft.title || ''} onChange={e => setField({ title: e.target.value })}
                      placeholder="Ex: Tecnologia que Impulsiona" style={{ height: 38 }} maxLength={200} />
                  </AdminField>
                  <AdminField label="Badge / Subtítulo" counter={{ current: (draft.subtitle || '').length, max: 500 }}>
                    <Input value={draft.subtitle || ''} onChange={e => setField({ subtitle: e.target.value })}
                      placeholder="Frase de destaque" style={{ height: 38 }} maxLength={500} />
                  </AdminField>
                </div>
                <div style={{ marginBottom: 14 }}>
                  <AdminField label="Descrição" hint="Texto de apoio ao título — 1 ou 2 frases.">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  ...
                  <AdminField label="Link do Botão" hint="Caminho interno (/solucoes) ou URL completa">
              ...
              {/* ── Configurações ── */}
              <AdminCard title="Configurações" description="Página onde aparece e ordem dentro dela.">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <AdminField label="Página">
                  ...
                  <AdminField label="Ordem" hint="Banners com ordem menor aparecem primeiro.">
