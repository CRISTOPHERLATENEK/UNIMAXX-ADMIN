  ...
  return (
    <div data-reveal="up" style={{ padding: '48px 0' }}>
      <div style={{ maxWidth: '80rem', margin: '0 auto', padding: '0 clamp(1rem,3vw,2rem)' }}>
        <div ref={ref} className="numbers-dark-card" style={{ position: 'relative', overflow: 'hidden', borderRadius: 28, padding: 'clamp(2rem,6vw,72px) clamp(1.25rem,4vw,48px)', background: 'var(--s0)', border: '1px solid var(--b1)', boxShadow: '0 32px 80px rgba(0,0,0,.35)' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(circle,var(--b1) 1px,transparent 1px)', backgroundSize: '44px 44px', pointerEvents: 'none' }} />
          <div style={{ position: 'relative', zIndex: 1 }}>
            {header}
            <div className="numbers-grid-3" style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fit, minmax(min(100%, 220px), 1fr))`, gap: 16, maxWidth: '56rem', margin: '0 auto' }}>
...
function StatCardDark({ stat, pc, delay, inView }: any) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ position: 'relative', borderRadius: 20, padding: 'clamp(1.25rem,3.5vw,40px) clamp(1rem,3vw,32px)', textAlign: 'center', background: hov ? 'rgba(255,255,255,.07)' : 'rgba(255,255,255,.04)', border: `1px solid ${hov ? pc + '30' : 'rgba(255,255,255,.07)'}`, boxShadow: hov ? `0 20px 60px ${pc}10` : 'none', opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(24px)', transition: `opacity .6s ${delay}s ease, transform .6s ${delay}s ease, background .3s, border-color .3s, box-shadow .3s` }}>
      <div style={{ fontSize: 'clamp(2.8rem,5vw,4.2rem)', fontWeight: 'var(--typo-h-weight,900)', marginBottom: 8, background: `linear-gradient(135deg,var(--t1),${pc})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontFamily: "var(--font-heading,'Outfit'),sans-serif", letterSpacing: '-0.04em', lineHeight: 1 }}>{stat.value}</div>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.16em', fontFamily: "var(--font-body,'DM Sans'),sans-serif" }}>{stat.label}</div>
...
function LayoutLight({ stats, pc, header }: any) {
  ...
  return (
    <div ref={ref} style={{ padding: '80px 0', background: 'var(--s0)' }}>
      <div style={{ maxWidth: '80rem', margin: '0 auto', padding: '0 clamp(1rem,3vw,2rem)' }}>
        {header}
        <div className="numbers-grid-4" style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fit, minmax(min(100%, 200px), 1fr))`, gap: 0 }}>
          {stats.map((s: any, i: number) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: `repeat(auto-fit, minmax(min(100%, 200px), 1fr))`, gap: 0 }}>
          {stats.map((s: any, i: number) => (
            <div key={i} style={{ padding: '32px 24px', textAlign: 'center', borderRight: i < stats.length - 1 ? '1px solid rgba(255,255,255,.2)' : 'none', opacity: inView ? 1 : 0, transform: inView ? 'scale(1)' : 'scale(.9)', transition: `opacity .5s ${i * 0.1}s, transform .5s ${i * 0.1}s` }}>
              <div style={{ fontSize: 'clamp(2.8rem,5vw,4rem)', fontWeight: 'var(--typo-h-weight,900)', color: '#fff', fontFamily: "var(--font-heading,'Outfit'),sans-serif", letterSpacing: '-0.04em', lineHeight: 1, marginBottom: 10 }}>{s.value}</div>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,.75)', textTransform: 'uppercase', letterSpacing: '0.14em', fontFamily: "var(--font-body,'DM Sans'),sans-serif" }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '48px 64px' }}>
          {stats.map((s: any, i: number) => (
            <div key={i} style={{ textAlign: 'center', opacity: inView ? 1 : 0, transform: inView ? 'none' : 'translateY(20px)', transition: `opacity .6s ${i * 0.15}s, transform .6s ${i * 0.15}s` }}>
              <div style={{ fontSize: 'clamp(3.5rem,6vw,6rem)', fontWeight: 'var(--typo-h-weight,900)', color: 'var(--t1)', fontFamily: "var(--font-heading,'Outfit'),sans-serif", letterSpacing: '-0.06em', lineHeight: 1 }}>
                <span style={{ color: pc }}>{s.value}</span>
              </div>
              <div style={{ marginTop: 8, fontSize: 12, fontWeight: 600, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.16em', fontFamily: "var(--font-body,'DM Sans'),sans-serif" }}>{s.label}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <div style={{ textAlign: 'center', marginBottom: 48 }}>
      {content['stats.title'] && (
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, marginBottom: 16, padding: '4px 14px', borderRadius: 99, border: `1px solid ${layout === 'dark' ? pc + '30' : pc + '40'}`, background: layout === 'dark' ? `${pc}12` : `${pc}10` }}>
          ...
          <span style={{ fontSize: 10.5, fontWeight: 700, color: pc, letterSpacing: '0.14em', textTransform: 'uppercase', fontFamily: "var(--font-body,'DM Sans'),sans-serif" }}>{content['stats.title']}</span>
        </div>
      )}
      {content['stats.subtitle'] && (
        <h2 style={{ fontFamily: "var(--font-heading,'Outfit'),sans-serif", fontWeight: 'var(--typo-h-weight,900)', fontSize: 'clamp(1.8rem,4vw,3rem)', letterSpacing: '-0.03em', color: 'var(--t1)', margin: '0 0 12px', lineHeight: 1.1 , ...bt.h}}>{content['stats.subtitle']}</h2>
      )}
      {content['stats.description'] && (
        <p style={{ fontFamily: "var(--font-body,'DM Sans'),sans-serif", fontSize: 15, lineHeight: 1.75, color: 'var(--t3)', margin: 0, maxWidth: 440, marginInline: 'auto' , ...bt.body}}>{content['stats.description']}</p>
      )}
    </div>
  ) : null;
