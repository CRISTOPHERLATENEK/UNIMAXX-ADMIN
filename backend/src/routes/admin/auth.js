...
router.post('/login', (req, res) => {
  ...
  db.get('SELECT * FROM users WHERE email=?', [email], (err, user) => {
    ...
    const refreshToken = jwt.sign({ id: user.id, email: user.email, name: user.name, type: 'refresh' }, JWT_SECRET, { expiresIn: '7d' });
...
router.post('/refresh', (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: payload.id, email: payload.email, name: payload.name }, JWT_SECRET, { expiresIn: '2h' });
    const newRefresh = jwt.sign({ id: payload.id, email: payload.email, name: payload.name, type: 'refresh' }, JWT_SECRET, { expiresIn: '7d' });
...
router.put('/profile', authenticateToken, (req, res) => {
  ...
  db.get('SELECT id FROM users WHERE email=? AND id!=?', [email, req.user.id], (err, conflict) => {
    ...
    db.run('UPDATE users SET name=?, email=? WHERE id=?', [name, email, req.user.id], function(err2) {
      ...
      const token = jwt.sign({ id: req.user.id, email, name }, JWT_SECRET, { expiresIn: '2h' });
